...
Use this when shipping a new `pi-moonshot` version.

## Checklist

1. Ensure the working tree is clean (or only includes intended release changes).
2. Confirm release version (`X.Y.Z`).
3. Run pre-release validation relevant to this repo (at minimum, ensure package metadata/files are correct and run `npm pack --dry-run`).
...
```bash
curl -s https://registry.npmjs.org/pi-moonshot \
